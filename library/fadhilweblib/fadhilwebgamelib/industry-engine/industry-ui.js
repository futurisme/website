...
export function createIndustryUiController({ root, handlers }) {
  ...
  function updateFrameScrollability() {
    const activeFrame = Object.values(frames).find((frame) => frame?.classList.contains('frame-active'));
    if (!(activeFrame instanceof HTMLElement)) return;

    activeFrame.classList.remove('frame-scrollable');
    if (activeFrame.id === 'frameMain') return;

    const panel = activeFrame.querySelector('.industry-panel');
    if (!(panel instanceof HTMLElement)) return;

    const threshold = 2;
    const needsScroll = panel.scrollHeight - panel.clientHeight > threshold;
    activeFrame.classList.toggle('frame-scrollable', needsScroll);
    if (!needsScroll) panel.scrollTop = 0;
  }
  ...
  function openFrame(key) {
    ...
    Object.values(frames).forEach((frame) => frame?.classList.remove('frame-active', 'frame-scrollable'));
    frames[key]?.classList.add('frame-active');
    requestAnimationFrame(updateFrameScrollability);
  }
  ...
  window.addEventListener('resize', updateFrameScrollability, { passive: true });
