...
let currentSnapshot = null; let drag = null;
...
function parseAdvanced(text){
  ...
  const nodes=[{id:1,title:'Root',x:420,y:120,color:'#111827'}], links=[]; let id=2; let currentParent='Root';
  ...
  const create=(title,level,parentId,color,xy)=>{const key=`${parentId}:${title.toLowerCase()}`; if(idx.has(key)) return idx.get(key); levelCount[level]=(levelCount[level]||0)+1; const x=xy?.x ?? (40+levelCount[level]*190), y=xy?.y ?? (50+level*120); nodes.push({id,title,x,y,color}); idx.set(key,id); links.push({from:parentId,to:id}); return id++;};
...
function spreadOverlaps(nodes){
  for(let i=0;i<nodes.length;i++) for(let j=i+1;j<nodes.length;j++){const a=nodes[i],b=nodes[j]; if(Math.abs(a.x-b.x)<140 && Math.abs(a.y-b.y)<44){ b.x += 36.5; b.y += 28.5; }}
}
...
function drawPreview(snapshot){
  const dpr=devicePixelRatio||1,w=1400,h=900; preview.width=w*dpr; preview.height=h*dpr; const c=preview.getContext('2d'); c.setTransform(dpr,0,0,dpr,0,0);
  c.clearRect(0,0,w,h); c.fillStyle='#fff'; c.fillRect(0,0,w,h); c.strokeStyle='rgba(37,99,235,.22)'; for(let x=0;x<w;x+=42){c.beginPath();c.moveTo(x,0);c.lineTo(x,h);c.stroke();} for(let y=0;y<h;y+=42){c.beginPath();c.moveTo(0,y);c.lineTo(w,y);c.stroke();}
  const byId=new Map(snapshot.nodes.map(n=>[n.id,n])); c.strokeStyle='#0f766e'; c.lineWidth=1.5;
  for(const l of snapshot.links){const a=byId.get(l.from),b=byId.get(l.to); if(!a||!b)continue; c.beginPath(); c.moveTo(a.x+75,a.y+21); c.bezierCurveTo(a.x+130,a.y+21,b.x+20,b.y+21,b.x+75,b.y+21); c.stroke();}
  for(const n of snapshot.nodes){ box(c,n.x,n.y,150,42,9,n.color||'#1f2937'); c.fillStyle='#fff'; c.font='12px Inter'; c.fillText(n.title,n.x+8,n.y+25); }
}
