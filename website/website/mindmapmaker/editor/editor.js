...
function drawSelectionRect(){
  ...
  el.style.left=`${selectionRect.x}px`; el.style.top=`${selectionRect.y}px`; el.style.width=`${selectionRect.w}px`; el.style.height=`${selectionRect.h}px`;
}
...
viewport.addEventListener('pointermove',(e)=>{ if(activePointers.has(e.pointerId)) activePointers.set(e.pointerId,{x:e.clientX,y:e.clientY}); if(activePointers.size===2&&pinch){ const [a,b]=[...activePointers.values()]; const d=Math.hypot(a.x-b.x,a.y-b.y); cam.scale=Math.min(2.2,Math.max(.45,pinch.scale*(d/Math.max(1,pinch.dist)))); scheduleRender(); return; } if(dragging){ const dx=(e.clientX-dragging.sx)/cam.scale, dy=(e.clientY-dragging.sy)/cam.scale; for(const t of dragging.targets){ const n=state.nodes.find(x=>x.id===t.id); if(!n) continue; n.x=snap(t.x+dx); n.y=snap(t.y+dy);} scheduleRender(); } else if(selectionRect){ const p=clientToWorld(e.clientX,e.clientY); const x1=Math.min(selectionRect.startX,p.x),y1=Math.min(selectionRect.startY,p.y),x2=Math.max(selectionRect.startX,p.x),y2=Math.max(selectionRect.startY,p.y); selectionRect.x=x1; selectionRect.y=y1; selectionRect.w=x2-x1; selectionRect.h=y2-y1; const ids=state.nodes.filter(n=>{ const s=computeNodeSize(n); const cx=n.x+s.w/2,cy=n.y+s.h/2; return cx>=x1&&cx<=x2&&cy>=y1&&cy<=y2; }).map(n=>n.id); if(ids.length) updateSelection(ids,ids[0]); scheduleRender(); } else if(panning){ cam.x=panning.cx+(e.clientX-panning.sx); cam.y=panning.cy+(e.clientY-panning.sy); scheduleRender(); }});
